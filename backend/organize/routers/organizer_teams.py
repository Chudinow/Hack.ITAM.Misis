...
async def export_teams_csv_download(
    ...
    with tempfile.NamedTemporaryFile(mode='w', suffix='.csv', delete=False, encoding='utf-8') as tmp_file:
        ...
        writer.writerow([
            "ID команды", "Название команды", "Завершена", 
            "ID участника", "Имя участника", "Роль", "Одобрен",
            "ID хакатона", "Название хакатона"
        ])
...
@router.post(
    ...
    responses={
        ...
        403: {"model": ErrorResponse, "description": "Нет прав доступа к хакатону"},
        404: {"model": ErrorResponse, "description": "Хакатон, команда или пользователь не найдены"}
    }
)
